...
async function seed() {
    ...
    const org = await prisma.organization.create({
        data: {
            ...
            members: {
                create: [
                    { userId: adminUser.id, role: client_1.Role.OWNER },
                    { userId: devUser.id, role: client_1.Role.MEMBER }
                ]
            }
        }
    });
    ...
    const linearPolicy = await prisma.retryPolicy.create({
        data: {
            ...
            type: client_1.RetryPolicyType.LINEAR,
            maxAttempts: 4,
            baseDelaySeconds: 15,
            maxDelaySeconds: 120
        }
    ...
    await prisma.jobLog.createMany({
        data: [
            { executionId: compExec.id, level: client_1.LogLevel.INFO, message: 'Initiating send_email handler execution' },
            { executionId: compExec.id, level: client_1.LogLevel.INFO, message: 'Connecting to SMTP gateway mail.acme.com:587' },
            { executionId: compExec.id, level: client_1.LogLevel.INFO, message: 'Email successfully accepted by relay host with ID msg-abc-12345' }
        ]
    });
    ...
    await prisma.jobLog.createMany({
        data: [
            { executionId: failedExec.id, level: client_1.LogLevel.INFO, message: 'Posting HTTP payload to https://api.thirdparty.org/hook' },
            { executionId: failedExec.id, level: client_1.LogLevel.ERROR, message: 'HTTP request failed with status code 503' },
            { executionId: failedExec.id, level: client_1.LogLevel.WARN, message: 'Scheduling attempt #2 with fixed 10s retry policy' }
        ]
    });
    // 5. Dead-Lettered Job
